...
const EVENTS: readonly EventItem[] = [
  ...
  { id: 5, title: "Compliance Training",       date: "02 Jun 2026", time: "03:00 PM", location: "Training Room B",    url: "/#", color: "bg-rose-500" },
...
export const Event: React.FC<EventProps> = ({
  ...
  const items = EVENTS.slice(0, Math.max(1, Math.min(maxItems, EVENTS.length)));
    ...
    <div className="card h-full flex flex-col">
      ...
      <ul className="flex flex-col gap-3 flex-1 overflow-y-auto max-h-[560px] pr-0.5">
        {items.map((item) => (
          ...
          >
            <div className={`${item.color} text-white rounded-xl w-12 flex-shrink-0 flex flex-col items-center justify-center py-2 text-center`}>
              <span className="text-[10px] font-semibold uppercase leading-none">{item.date.split(" ")[1]}</span>
              <span className="text-xl font-bold leading-none mt-0.5">{item.date.split(" ")[0]}</span>
            </div>
            <div className="flex-1 min-w-0 py-1">
              <p className={`text-sm font-medium text-text line-clamp-1 ${!editMode ? "group-hover:text-primary" : ""} transition-colors`}>
                {item.title}
              </p>
              <p className="text-[11px] text-text-muted mt-0.5">{item.time} · {item.location}</p>
            </div>
          </li>
        ))}
      </ul>
